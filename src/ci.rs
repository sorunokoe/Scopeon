...
fn render_report(current: &CiSnapshot, baseline: Option<&CiSnapshot>) -> String {
    ...
    if let Some(base) = baseline {
        let current_day = prefix_chars(&current.captured_at, 10);
        let baseline_day = prefix_chars(&base.captured_at, 10);
        out.push_str(&format!(
    ...
    let captured_at = prefix_chars(&current.captured_at, 19);
    out.push_str(&format!(
...
fn prefix_chars(input: &str, max_chars: usize) -> String {
